...
const FLAT_DELIM = '~'; // delimiter for flat hierarchy
const TRIM_CHAR = " \t\n\r\0\x0B";
const NEWL = "\n";      // for csv file export
const ENCL = '"';
...
public function getTreeNode(&$array, $nd) {
	if (is_array($array)){
		foreach ($array as $k =>$v){
			...
			} else $nods = $this->getTreeNode($v,$nd);
		if(isset($nods)) return $nods;
		}
	}
}
...
private function mergeBranch(&$arr, $el, $br){
	if (is_array($arr)){
		foreach ($arr as $k => $v){
			...
			return ;
			} else	$this->mergeBranch($arr[$k], $el, $br);
		}
	}
	return;
}
...
public function displayHierarchyTable($altNum, $altButton, $ahp = true, $pflag = true){
	...
	if($pflag)
		echo "<th>" . $this->ahpHierTxt->wrd['glbP'] . "</th>"; // global priorities
	for ($j=0; $j< $altNum; $j++)
