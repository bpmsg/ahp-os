...
public function set_txtbuf($ds){
	...
	// --- Title
	$txtbuf .= $this->header . $fs . self::ENCL . date("Y.m.d H:i:s"). self::ENCL.self::NEWL;
	$txtbuf .= $n;
	$txtbuf .= self::NEWL;
	...
	$txtbuf .= rtrim($cbuf,$fs) . self::NEWL . $mbuf . rtrim($ebuf, $fs) . self::NEWL;
	$txtbuf .= number_format($this->evm_eval, ROUND, $ds, $ts) . $fs 
		. number_format($this->cr_alo, ROUND, $ds, $ts) . self::NEWL;
//	$txtbuf .= self::ENCL . $this->getNodeTxt() . self::ENCL . self::NEWL;
...
public function printVector($names, $vector, $reverse, $tol=array()){
	...
	if (!is_array($vector)) return 1;
	if(count($tol)>1) 
		$tflg=true;
	else
		$tflg = false;
	$n = count($vector);
	//	$csc = $this->colors->hueScale( $rgbBaseColor, $rgbEndColor, $n);
