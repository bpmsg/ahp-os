 ...
 */
	public function __construct($sc){
		...
		if( $this->ahpDb->checkSessionCode($sc)){
			...
			if(empty($this->part)){
				$this->err[] = $this->ahpGroupTxt->wrn['noPart']; // Project has no participants
			} else {
			...
			$this->err[] = $this->ahpGroupTxt->err['invSc']; // Invalid Session code
			$this->sessionCode = "";
		}
	}
...
public function uncertainty($pwcr, $ahpH, $iScale, $sdFlg=0, $hierMode){
	...
	if($hierMode){
		...
		foreach($this->leafs as $node)
			$ahpH->prioAlt[$node] = array_values($this->prio[0][$node]); // nom
