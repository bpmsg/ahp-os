...
*/
    public $user_name = "";
    /**
* @var  string $refUrl referring url - added by Klaus
*/
    public $refUrl = "";
    /**
    ...
    public function databaseConnection()
    {
        ...
        if ($this->db_connection != null) {
            ...
            try {
                ...
                $this->errors[] = $this->lgTxt->err['dbCon'] . $e->getMessage(); // MESSAGE_DATABASE_ERROR
            }
        }
        // default return
        return false;
...
*/
    private function loginWithCookieData()
    {
        if (isset($_COOKIE['rememberme'])) {
            ...
            $this->errors[] = $this->lgTxt->err['iCk']; // MESSAGE_COOKIE_INVALID
        }
        return false;
    }
...
*/
    ...
    {
        if (empty($user_name)) {
            $this->errors[] = $this->lgTxt->err['unE']; // MESSAGE_USERNAME_EMPTY
        } else if (empty($user_password)) {
            $this->errors[] = $this->lgTxt->err['pwE']; // MESSAGE_PASSWORD_EMPTY
        ...
        // if POST data (from login form) contains non-empty user_name and non-empty user_password
            ...
            if (! isset($result_row->user_id)) {
              	...
              	$this->errors[] = $this->lgTxt->err['unNe']; // MESSAGE_USER_DOES_NOT_EXIST
            } else if (($result_row->user_failed_logins >= 3) && ($result_row->user_last_failed_login > (time() - 30))) {
              	$this->errors[] = $this->lgTxt->err['pwW3']; // MESSAGE_PASSWORD_WRONG_3_TIMES
            // using PHP 5.5's password_verify() function to check if the provided passwords fits to the hash of that user's password
            } else if (! password_verify($user_password, $result_row->user_password_hash)) {
                // increment the failed login counter for that user
                $sth = $this->db_connection->prepare("UPDATE users SET user_failed_logins = user_failed_logins+1, 
								user_last_failed_login = :user_last_failed_login 
								WHERE user_name= :user_name OR user_email= :user_email;");
                $sth->execute(array(':user_name' => $user_name, ':user_email' => $user_name, ':user_last_failed_login' => time()));
                $this->errors[] = $this->lgTxt->err['pwW']; // MESSAGE_PASSWORD_WRONG
            // has the user activated their account with the verification email
            } else if ($result_row->user_active != 1) {
                $this->errors[] = $this->lgTxt->err['aNact']; // MESSAGE_ACCOUNT_NOT_ACTIVATED
            } else {
                // write user data into PHP SESSION [a file on your server]
                $_SESSION['user_id'] = $result_row->user_id;
                $_SESSION['user_name'] = $result_row->user_name;
...
public function doLogout()
{
	...
	if ( $this->databaseConnection() && isset($_SESSION['user_logged_in'])){
	    ...
	    VALUES ('U', '" . $_SESSION['user_id'] . "','" . $_SESSION['user_name'] . "','Logout');";
	    $insert = $this->db_connection->prepare($sql);
	    $insert->execute();
    } catch (PDOException $e){
			$this->errors[] = $this->lgTxt->err['dbCon'] . $e; // MESSAGE_DATABASE_ERROR
    } 
  }
  $this->deleteRememberMeCookie();
 ...
 */
  ...
  $this->messages[] = $this->lgTxt->msg['lgOut']; // MESSAGE_LOGGED_OUT
...
*/
    public function editUserName($user_name)
    {
        ...
        if (!empty($user_name) && $user_name == $_SESSION['user_name']) {
            $this->errors[] = $this->lgTxt->err['unDb']; // MESSAGE_USERNAME_SAME_LIKE_OLD_ONE
        ...
        // TODO: maybe this pattern should also be implemented in Registration.php (or other way round)
            ...
            $this->errors[] = $this->lgTxt->err['unIv']; // MESSAGE_USERNAME_INVALID
            ...
            if (isset($result_row->user_id)) {
                $this->errors[] = $this->lgTxt->err['unTk']; // MESSAGE_USERNAME_EXISTS
            } else {
                ...
                if ($query_edit_user_name->rowCount()) {
                    ...
                    $this->messages[] = $this->lgTxt->msg['unCok'] . $user_name; // MESSAGE_USERNAME_CHANGED_SUCCESSFULLY
                } else {
                    $this->errors[] = $this->lgTxt->err['unF']; // MESSAGE_USERNAME_CHANGE_FAILED
                }
            }
        }
    }
...
*/
    public function editUserEmail($user_email)
    {
        ...
        if (!empty($user_email) && $user_email == $_SESSION["user_email"]) {
            $this->errors[] = $this->lgTxt->err['emlD']; // MESSAGE_EMAIL_SAME_LIKE_OLD_ONE
        // user mail cannot be empty and must be in email format
        } elseif (empty($user_email) || !filter_var($user_email, FILTER_VALIDATE_EMAIL)) {
            $this->errors[] = $this->lgTxt->err['emlI']; // MESSAGE_EMAIL_INVALID
            ...
            if (isset($result_row->user_id)) {
                $this->errors[] = $this->lgTxt->err['emlR']; // MESSAGE_EMAIL_ALREADY_EXISTS
            } else {
                ...
                if ($query_edit_user_email->rowCount()) {
                    ...
                    $this->messages[] = sprintf($this->lgTxt->msg['emlCok'],$user_email); // MESSAGE_EMAIL_CHANGED_SUCCESSFULLY
                } else {
                    $this->errors[] = $this->lgTxt->err['emlNc']; // MESSAGE_EMAIL_CHANGE_FAILED
                }
            }
        }
    }
...
*/
    public function editUserPassword($user_password_old, $user_password_new, $user_password_repeat)
    {
        ...
        // is the repeat password identical to password
            ...
            $this->errors[] = $this->lgTxt->err['pwNi']; // MESSAGE_PASSWORD_BAD_CONFIRM
        // password need to have a minimum length of 6 characters
        } elseif (strlen($user_password_new) < 6) {
            $this->errors[] = $this->lgTxt->err['pwS']; // MESSAGE_PASSWORD_TOO_SHORT
            ...
            if (isset($result_row->user_password_hash)) {
                ...
                if (password_verify($user_password_old, $result_row->user_password_hash)) {
                    ...
                    if ($query_update->rowCount()) {
                        $this->messages[] = $this->lgTxt->msg['pwCok']; // MESSAGE_PASSWORD_CHANGED_SUCCESSFULLY
                    } else {
                        $this->errors[] = $this->lgTxt->err['pwCf'] ; // MESSAGE_PASSWORD_CHANGE_FAILED
                    }
                } else {
                    $this->errors[] = $this->lgTxt->err['pwOw']; // MESSAGE_OLD_PASSWORD_WRONG
                }
            } else {
                $this->errors[] = $this->lgTxt->err['unNe']; // MESSAGE_USER_DOES_NOT_EXIST
            }
...
*/
    public function setPasswordResetDatabaseTokenAndSendMail($user_email)
    {
        ...
        if (empty($user_email)) {
            ...
            // database query, getting all the info of the selected user
						// -- changed to get info from email
						// $result_row = $this->getUserData($user_name);
            ...
            if (isset($result_row->user_id)) {
                ...
                if ($query_update->rowCount() == 1) {
                    ...
                    $this->errors[] = $this->lgTxt->err['dbCon']; // MESSAGE_DATABASE_ERROR
                }
            } else {
                $this->errors[] = $this->lgTxt->err['unNe']; // MESSAGE_USER_DOES_NOT_EXIST
            }
...
*/
    public function sendPasswordResetMail($user_name, $user_email, $user_password_reset_hash)
    {
        ...
        if(!$mail->Send()) {
            $this->errors[] = $this->lgTxt->err['pwCf'] . $mail->ErrorInfo; // MESSAGE_PASSWORD_RESET_MAIL_FAILED
            return false;
        } else {
            $this->messages[] = $this->lgTxt->msg['pwRms']; // MESSAGE_PASSWORD_RESET_MAIL_SUCCESSFULLY_SENT
            return true;
        }
    }
...
*/
    public function checkIfEmailVerificationCodeIsValid($user_name, $verification_code)
    {
        ...
        if (empty($user_name) || empty($verification_code)) {
            $this->errors[] = $this->lgTxt->err['lnkE']; // MESSAGE_LINK_PARAMETER_EMPTY 
        } else {
            // database query, getting all the info of the selected user
            $result_row = $this->getUserData($user_name);
            ...
            if (isset($result_row->user_id) && $result_row->user_password_reset_hash == $verification_code) {
                ...
                if ($result_row->user_password_reset_timestamp > $timestamp_one_hour_ago) {
                    ...
                    $this->errors[] = $this->lgTxt->err['lnkExp']; // MESSAGE_RESET_LINK_HAS_EXPIRED
                }
            } else {
                $this->errors[] = $this->lgTxt->err['unNe']; // MESSAGE_USER_DOES_NOT_EXIST
            }
    ...
    public function editNewPassword($user_name, $user_password_reset_hash, $user_password_new, $user_password_repeat)
    {
        ...
        // if database connection opened
            ...
            if ($query_update->rowCount() == 1) {
                ...
                $this->errors[] = $this->lgTxt->msg['pwCf']; // MESSAGE_PASSWORD_CHANGE_FAILED
            }
        }
    }
