...
/*
 * Validation of form data. This function can serve as a template for 
 * other forms
 * @version 2017-05-17
...
function validateFormData($trD = array())
{
    ...
    $fData['trName'] 
		= mb_substr(preg_replace('~[^\p{L}\p{N}\-\&]++~u', ' ', $fData['trName']), 0, 64);
    //	if(filter_has_var(INPUT_POST, 'trUser'))
    $fData['trUser'] 
		= mb_substr(preg_replace('~[^\p{L}\p{N}\-\&]++~u', ' ', $fData['trUser']), 0, 64);
    //	if(filter_has_var(INPUT_POST, 'trId'))
    $fData['trId'] 
		= mb_substr(preg_replace('~[^\p{L}\p{N}\-\&]++~u', ' ', $fData['trId']), 0, 30);
    //	if(filter_has_var(INPUT_POST, 'trCmnt'))
    $fData['trCmnt'] 
		= mb_substr(preg_replace('~[^\p{L}\p{N}\-\&\?\.\!\,\:]++~u', ' ', $fData['trCmnt']), 0, 255);
    $fData['trAmnt'] = mb_substr(filter_input(INPUT_POST,
     'trAmnt', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION), 0, 10);
    $fData['trFee'] =  mb_substr(filter_input(INPUT_POST,
     'trFee', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION), 0, 10);
    $fData['trFx'] =   mb_substr(filter_input(INPUT_POST,
     'trFx', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION), 0, 10);
    $fData['trEmail']= mb_substr(filter_input(INPUT_POST,
     'trEmail', FILTER_SANITIZE_EMAIL), 0, 64);
    ...
    if ($fData['trNo'] != 0 && ! empty($trD)) {
        ...
        if (!isset($fData['trAmnt']) || $fData['trAmnt'] == "") {
            $fData['trAmnt'] = $trD['trAmnt'];
        }
        if (!isset($fData['trFee']) || $fData['trFee'] == "") {
            $fData['trFee']  = $trD['trFee'];
        }
        //		if($fData['trDate'] == date('Y-m-d'))
        //			$fData['trDate'] = $trD['trDate'];
        if ($fData['trName'] == "") {
            $fData['trName'] = $trD['trName'];
        }
        if ($fData['trEmail'] == "") {
            $fData['trEmail'] = $trD['trEmail'];
        }
        if ($fData['trCmnt'] == "") {
            $fData['trCmnt'] = $trD['trCmnt'];
        }
    ...
    if (isset($_POST['REFR'])) {
        ...
        if (isset($_POST['CLR'])) {
            $para['trName'] = $para['trEmail'] = $para['trId'] 
                            = $para['trCmnt'] = $para['trFx'] 
                            = $para['trAmnt'] = $para['trFee'] 
                            = $para['trUser'] = $para['trUid'] 
                            = $para['trUemail'] = "";
            $para['trNo'] = 0;
            $para['trDate'] = date('Y-m-d');
        }
        if ((isset($_POST['CHKID']) || isset($_POST['UPDT']) 
            || isset($_POST['MOD']) || isset($_POST['CLR']))) {
            ...
            if (count($donor) > 0) {
                $msg = "<span class='hl'>We have " . count($donor) 
                . " donation(s) from this user already.</span>";
                if ($para['trName'] == "") {
        ...
        if (isset($_POST['UPDT']) && count($ahpAdmin->errors)==0 && $err == "") {
            $flag = $ahpAdmin->writeUserDonation(
                $para['trDate'],
                $para['trId'],
                $para['trAmnt'],
                $para['trFee'],
