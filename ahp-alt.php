...
// -- ALTNUM, PROJECT, ALTERNATIVES from GET PARAMETER
	$getErr = $ahp->setNamesFromGet( $ahpH->altNum, $ahpH->project, $ahpH->alt, ALTAHP, "Alt-");
...
// -- SUBMIT DATA FOR GROUP EVALUATION 'group' -
		if(isset($_POST['group'])) {
			if(isset($_SESSION['pwc'])){
				$ahpDb = new AhpDb();
				$insState = $ahpDb->submitGroupData($_SESSION['sessionCode'], $_SESSION['name'], $_SESSION['pwc']);
				// store pwc in session file
				if( !empty($insState) ){
					if($insState['upd'] >0)
						$checkWarn = sprintf($ahpPrioCalc->msg['giUpd'], $insState['upd']);
					if($insState['ins'] >0)
						$checkWarn = sprintf($ahpPrioCalc->msg['giUpd'], $insState['ins']);
					if( $ahpH->pwcaDoneFlg){
						$leave = true;
